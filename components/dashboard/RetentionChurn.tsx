        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <p className="text-xs text-slate-500">
              Retention is the share of customers with more than one job in the range. “Churn” style rows count open jobs by treatment until you log cancellation reasons on jobs.
            </p>
            {reasons.length === 0 ? (
              <p className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                No open-job clusters in this period — nothing to list here.
              </p>
            ) : (
              reasons.map((entry) => (
                <div key={entry.reason} className="rounded-3xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <span className="text-sm text-slate-600">{entry.count} jobs</span>
                  </div>
